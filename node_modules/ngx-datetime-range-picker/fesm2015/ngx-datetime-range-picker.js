import { ɵɵdefineInjectable, Injectable, EventEmitter, Component, ViewEncapsulation, ElementRef, Renderer2, Input, Output, ViewChild, NgModule, Pipe, Optional, SkipSelf } from '@angular/core';
...
NgxDatetimeRangePickerService.ɵprov = ɵɵdefineInjectable({ factory: function NgxDatetimeRangePickerService_Factory() { return new NgxDatetimeRangePickerService(); }, token: NgxDatetimeRangePickerService, providedIn: "root" });
...
const USA_TZ_CODE$1 = NgxDatetimeRangePickerConstants.CONSTANT.USA_TZ_CODE;
class NgxDatetimeRangePickerComponent {
    ...
    onClickPrevious(side) {
        const { label, labelFormat, type } = this.service.getLabelProps(this.state, this.config.type, side);
        const startDate = moment$2(label, labelFormat)
            .subtract(1, type)
            .startOf(type)
            .format(DEFAULT_DATE_FORMAT);
    ...
    onClickNext(side) {
        const { label, labelFormat, type } = this.service.getLabelProps(this.state, this.config.type, side);
        const endDate = moment$2(label, labelFormat)
            .add(1, type)
            .endOf(type)
            .format(DEFAULT_DATE_FORMAT);
    ...
    onCellClick(item, itemCell, side) {
        const date = moment$2(item.date, DEFAULT_DATE_FORMAT).valueOf();
        const startDate = moment$2(this.config.startDate, DEFAULT_DATE_FORMAT).valueOf();
        const endDate = moment$2(this.config.endDate, DEFAULT_DATE_FORMAT).valueOf();
        const minDate = moment$2(this.config.minDate, DEFAULT_DATE_FORMAT).valueOf();
        const maxDate = moment$2(this.config.maxDate, DEFAULT_DATE_FORMAT).valueOf();
        if (!item.available) {
            if (date < minDate || date > maxDate) {
                return;
            }
        ...
        if (!endDate) {
            const func = (rowItem) => {
                if (rowItem.available) {
                    const hoverItemDate = rowItem.date ? moment$2(rowItem.date, DEFAULT_DATE_FORMAT).valueOf() : rowItem.date;
    ...
    onCalendarLabelChange(label, side, type) {
        ...
        if (this.config.type !== "yearly") {
            ...
            const date = moment$2(selectedMonth, "MMM YYYY")
                .startOf("month")
                .format(DEFAULT_DATE_FORMAT);
    ...
    onTimeLabelChange(item, side, timeItem) {
        ...
        if (side === "left") {
            ...
            const startDateEpoch = moment$2(this.config.startDate, DEFAULT_DATE_FORMAT).valueOf();
            const endDateEpoch = moment$2(this.config.endDate, DEFAULT_DATE_FORMAT).valueOf();
    ...
    handleDateArray() {
        if (this.config.dateArray && this.config.dateArray.length) {
            ...
            this.config.dateArray = this.config.dateArray.sort((date1, date2) => {
                const value1 = moment$2(date1, DEFAULT_DATE_FORMAT).valueOf();
                const value2 = moment$2(date2, DEFAULT_DATE_FORMAT).valueOf();
    ...
    sanitizeDates() {
        ...
        const setDate = (type) => {
            this.config.minDate = moment$2(this.config.minDate, DEFAULT_DATE_FORMAT)
                .endOf(type)
                .format(DEFAULT_DATE_FORMAT);
            this.config.maxDate =
                type === "week"
                    ...
                    : moment$2(this.config.maxDate, DEFAULT_DATE_FORMAT)
                        .endOf(type)
                        .format(DEFAULT_DATE_FORMAT);
            this.config.startDate = moment$2(this.config.startDate, DEFAULT_DATE_FORMAT)
                .endOf(type)
                .format(DEFAULT_DATE_FORMAT);
            this.config.endDate =
                type === "week"
                    ? moment$2(this.config.endDate, DEFAULT_DATE_FORMAT)
                        .subtract(subtractWeekCount, "week")
                        .endOf(type)
                        .format(DEFAULT_DATE_FORMAT)
                    : moment$2(this.config.endDate, DEFAULT_DATE_FORMAT)
    ...
    generateCalendar(date, side) {
        ...
        if (this.config.type !== "yearly") {
            ...
            const monthStartWeekNumber = moment$2(date, DEFAULT_DATE_FORMAT)
                .year(this.state.selectedYear[side])
                .startOf("month")
                .week(); // previousMonthLastWeek
            const yearStartDate = moment$2(this.state.selectedYear[side], "YYYY")
                .startOf("year")
                .format(DEFAULT_DATE_FORMAT);
            let numberOfRows = 1;
            if (this.config.type === "daily") {
            ...
            for (let dateRows = 0; dateRows < numberOfRows; dateRows++) {
                ...
                const { rowNumber, columns } = this.service.getCalendarRowVariables(rowOptions);
                ...
                for (let rowItem = 0; rowItem <= columns; rowItem++) {
                    ...
                    const { currentItemDate, rowItemText, firstDay, lastDay, itemCount } = this.service.getCalendarRowItemVariables(rowItemOptions);
                    rowOptions.itemCount = itemCount;
                    const { available, inRange, active, today } = this.service.getDateCharacteristics(this.config, this.state, currentItemDate, calendarLabel, side);
                    const itemObj = {
    ...
    dateRangeSelected() {
        const dateRangeModel = this.getNgxDatetimeRangeChangeOutput();
        this.state.isCalendarVisible = false;
        this.filterInputBox.nativeElement.classList.remove("empty-filter");
        this.doDateRangeModelChange();
    ...
    getNgxDatetimeRangeChangeOutput(format) {
        let dateRangeChangeOutput;
        ...
        let endDate = moment$2(this.config.endDate, DEFAULT_DATE_FORMAT).format(outputDateFormat);
    ...
    doApply() {
        ...
        let outputStartDate = startDate ? moment$2(startDate, DEFAULT_DATE_FORMAT).valueOf() : null;
        let outputEndDate = endDate ? moment$2(endDate, DEFAULT_DATE_FORMAT).valueOf() : null;
    ...
    updateActiveItem() {
        const startDate = this.service.getFirstLastDay(this.config.startDate, this.config.type);
        const endDate = this.service.getFirstLastDay(this.config.endDate, this.config.type);
        if (this.config.type === "weekly") {
...
NgxDatetimeRangePickerComponent.ctorParameters = () => [
    { type: ElementRef },
    { type: Renderer2 },
...
NgxDatetimeRangePickerComponent.propDecorators = {
    options: [{ type: Input }],
    settings: [{ type: Input }],
    optionService: [{ type: Input }],
    dateRangeModel: [{ type: Input }],
    canBeEmpty: [{ type: Input }],
    dateRangeModelChange: [{ type: Output }],
    dateRangeChanged: [{ type: Output }],
    inputFocusBlur: [{ type: Output }],
    selectedDate: [{ type: Output }],
    filterInputBox: [{ type: ViewChild, args: ["filterInputBox", { static: false },] }]
};
...
ObjNgFor.decorators = [
    { type: Pipe, args: [{ name: "ObjNgFor", pure: false },] }
];

class NgxDatetimeRangePickerModule {
...
export { CalendarTypes, NgxDatetimeRangePickerComponent, NgxDatetimeRangePickerModule, NgxDatetimeRangePickerService, ɵ0, ObjNgFor as ɵa, MaterialModule as ɵb };
//# sourceMappingURL=ngx-datetime-range-picker.js.map
